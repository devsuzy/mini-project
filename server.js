...
//
// 실측 (qwen2.5:7b, 시스템 프롬프트 2,683자 + 대화 청크 6,475자):
//   입력 6,583 토큰 / 16,384  →  출력 여유 9,801 토큰
//   한국어는 자당 약 0.72 토큰이다(반복이 많은 글은 더 낮다).
// 여유가 꽤 남으므로 CHUNK_CHARS 를 늘릴 수는 있으나, 7B 는 컨텍스트가 길수록
// 눈에 띄게 느려지고 뒤쪽 필드를 성의 없이 채운다. 늘리기 전에 실제로 재볼 것.
const NUM_CTX = 16384;
